<script lang="ts">
  ...
  <h2
    ...
    class={cn("font-display text-center text-2xl font-bold tracking-[-0.02em] drop-shadow-sm md:text-4xl md:leading-[5rem]", className)}
    {...$$restProps}
  >
    <span class="relative inline-block">
      {#each sparkles as item}
        <Motion
          ...
          transition={{ duration: 0.8, repeat: "1s", delay: item.delay }}
          let:motion
        >
          <svg
            use:motion
